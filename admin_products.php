<?php
   ...
   if(isset($_POST['add_product'])){//thêm sách mới từ submit form name='add_product'
      ...
      if(mysqli_num_rows($select_product_name) > 0){
         $message[] = 'Bộ bài đã tồn tại.';
      }else{//chưa thì thêm mới
         $add_product_query = mysqli_query($conn, "INSERT INTO `products`(name, author, publisher, price, discount, newprice, quantity, initial_quantity, describes, image) VALUES('$name', '$author', '$publisher', '$price', '$discount', '$newprice', '$quantity', '$initial_quantity', '$describe', '$image')") or die('query failed');
         ...
         if($add_product_query){
            if($image_size > 2000000){//kiểm tra kích thước ảnh
               ...
               $message[] = 'Thêm bộ bài thành công!';
            }
         }else{
            $message[] = 'Thêm bộ bài không thành công!';
         }
      }
   }
   ...
   if(isset($_POST['update_product'])){//cập nhật sách từ form submit name='update_product'
      ...
      mysqli_query($conn, "UPDATE `products` SET name = '$update_name', author = '$update_author', price = '$update_price', newprice='$update_newprice', discount='$update_discount', quantity='$update_quantity', describes='$update_describe' WHERE id = '$update_p_id'") or die('query failed');
...
<section class="add-products">
   ...
   <h1 class="title">Bộ bài</h1>

   <form action="" method="post" enctype="multipart/form-data">
      <h3>Thêm bộ bài</h3>
      <input type="text" name="name" class="box" placeholder="Tên bộ bài" required>
      ...
      <input type="number" min="0" name="price" class="box" placeholder="Giá bộ bài" required>
   ...
   <div class="box-container">
      ...
      ?>
               <div class="box">
                  ...
                  <div class="sub-name">Nhà sản xuất: <?php echo $fetch_products['publisher']; ?></div>
                  ...
                  <a href="admin_products.php?delete=<?php echo $fetch_products['id']; ?>" class="delete-btn" onclick="return confirm('Xóa bộ bài này?');">Xóa</a>
               </div>
      <?php
            }
      }else{
         echo '<p class="empty">Không có bộ bài nào được thêm!</p>';
      }
      ?>
   </div>
...
<section class="edit-product-form">
   ...
   ?>
               <form action="" method="post" enctype="multipart/form-data">
                  ...
                  <input type="text" name="update_name" value="<?php echo $fetch_update['name']; ?>" class="box" required placeholder="Tên bộ bài">
                  <!-- <input type="text" name="update_author" value="<?php echo $fetch_update['author']; ?>" class="box" required placeholder="Tác giả"> -->
                  <input type="number" name="update_price" value="<?php echo $fetch_update['price']; ?>" min="0" class="box" required placeholder="Giá bộ bài">
                  ...
                  <input type="number" name="update_quantity" value="<?php echo $fetch_update['quantity']; ?>" min="0" class="box" required placeholder="Số lượng bộ bài">
